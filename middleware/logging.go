...
func LoggingMiddleware(logger *zap.Logger) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		// readable string field is bound only when a span exists, so no log line
		// claims a trace id the backend does not have.
		...
		// makes that contract true for logs too. A FAILING probe is kept — that
		...
		// which is what left the access log uncorrelated in 9 of the 10 HTTP
		// services (telemetry audit F-1). trace_id is already bound on it.
